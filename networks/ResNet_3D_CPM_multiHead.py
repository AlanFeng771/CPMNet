...
class Resnet18(nn.Module):
    ...
    def _init_weight(self):
        ...
        prior = 0.01
        nn.init.constant_(self.head1.cls_output.weight, 0)
        nn.init.constant_(self.head1.cls_output.bias, -math.log((1.0 - prior) / prior))

        nn.init.constant_(self.head1.shape_output.weight, 0)
        nn.init.constant_(self.head1.shape_output.bias, 0.5)

        nn.init.constant_(self.head1.offset_output.weight, 0)
        nn.init.constant_(self.head1.offset_output.bias, 0.05)
...
class DetectionLoss(nn.Module):
    ...
    def forward(self, 
        ...
        classification1_losses = self.cls_loss(pred1_scores, target_scores, mask_ignore.int(), num_hard=self.cls_num_hard, fn_weight=self.cls_fn_weight, fn_threshold=self.cls_fn_threshold)
        ...
        if fg_mask.sum() == 0:
            ...
            reg1_losses = torch.abs(pred1_shapes[fg_mask] - target_shape[fg_mask])
            offset1_losses = torch.abs(pred1_offsets[fg_mask] - target_offset[fg_mask])
            iou1_losses = - (self.bbox_iou(pred1_bboxes[fg_mask], target_bboxes[fg_mask]))
